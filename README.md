...
企业版增加账号权限、手册异步处理与任务状态，并使用 PostgreSQL、Redis、独立 Chroma 服务及持久化数据卷。推荐按 [企业版部署与备份恢复](docs/enterprise-deployment.md) 启动 Docker Compose；唯一浏览器入口是 `http://127.0.0.1:8080`。首次启动需先配置根目录 `.env`、执行迁移、准备本地 CPU 模型并创建管理员。

[企业版架构与演示](docs/enterprise-architecture.md) 说明服务分工与演示顺序。下面保留原检索演示和本地开发说明；新部署的数据卷为空，需重新导入已确认可用的资料。

## 项目亮点

- 按车型隔离检索：不会跨车型返回手册片段。
- PDF 解析、分块、向量化后写入本地 ChromaDB；回答只根据返回证据生成。
- 每条回答展示手册名称、章节、PDF 页码和原文节选；无可靠证据时拒绝编造。
- 检索结合多语言向量、BM25、RRF 融合、相似度阈值和引用页码去重。
- 内置离线基准题集，以 Recall@1、Recall@3、MRR、候选引用精度和拒答准确率评测检索质量。

## 系统架构

```mermaid
flowchart LR
    U[用户] --> F[React + Vite 前端]
    F -->|车型、问题、章节| A[FastAPI]
    A --> R[检索控制层]
    R --> V[Sentence Transformers 向量检索]
    R --> B[BM25 词法检索]
    V --> C[(ChromaDB)]
    B --> M[已导入 PDF 分块]
    C --> R
    R -->|最多 4 条证据| D[DeepSeek Chat]
    D -->|结构化回答与引用 ID| A
    A -->|答案、步骤、警告、页码引用| F
```
...
1. 选择本田思域或丰田卡罗拉。
2. 使用页面中的示例问题，或输入自定义问题。
3. 可选地勾选手册章节，缩小检索范围。
4. 查看回答、操作步骤、注意事项和带 PDF 页码的原文引用。

比亚迪海豹当前没有导入手册，适合作为“资料不足时安全拒答”的演示场景。
...
两种模式均最多返回 4 个证据块，距离阈值为 1.1。混合模式只会融合有正向 BM25 命中的词法结果；完全没有词法命中时会保留向量检索顺序。对于英文手册中的常见汽车术语，中文提问会附加有限的英文同义词（例如“发动机机油”→`engine oil`），不调用外部翻译服务。报告指标定义如下：
